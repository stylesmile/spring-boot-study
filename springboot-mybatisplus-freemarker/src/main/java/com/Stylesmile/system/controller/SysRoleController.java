...
public class SysRoleController {
    ...
    final String base_html_path = "/system/sysUser";
    @Autowired
    ...
    public Result edit(SysUser user) {
        ...
        //Boolean b2 = sysUserService.saveOrUpdate(user);
        if (b) {
            return Result.success();
        } else {
            return Result.fail();
...
//    public Result selectUserPage(Page<SysUser> page, Integer state) {
//        // 不进行 count sql 优化，解决 MP 无法自动优化 SQL 问题，这时候你需要自己查询 count 部分
//        // page.setOptimizeCountSql(false);
//        // 当 total 为非 0 时(默认为 0),分页插件不会进行 count 查询
//        // 要点!! 分页返回的对象与传入的对象是同一个
//        //IPage<SysUser> ipage=sysUserService.getUserList(page, state);
//        Page<SysUser> ipage=sysUserService.getUserList(page);
